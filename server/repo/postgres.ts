...
// This allows the gated test to load and skip cleanly when the env var is absent.
let _sql: ReturnType<typeof neon> | undefined;
function getSql(): ReturnType<typeof neon> {
  if (!_sql) {
    _sql = neon(); // reads NETLIFY_DATABASE_URL; throws if unset (only reached at runtime)
  }
  return _sql;
}
...
export async function ensureSchema(): Promise<void> {
  const sql = getSql();
  await sql`CREATE TABLE IF NOT EXISTS sessions (id TEXT PRIMARY KEY, created_at TIMESTAMPTZ DEFAULT now())`;
...
export function createPostgresRepository(): Repository {
  return {
    async createSession(id) {
      const sql = getSql();
      await sql`INSERT INTO sessions (id) VALUES (${id}) ON CONFLICT DO NOTHING`;
    },
    async hasSession(id) {
      const sql = getSql();
      const r = await sql`SELECT 1 FROM sessions WHERE id = ${id}`;
      return r.length > 0;
    },
    ...
    async updateLead(id, patch) {
      ...
      const n = { ...cur, ...patch };
      const sql = getSql();
      await sql`UPDATE leads SET status=${n.status}, envelope_id=${n.envelopeId ?? null}, signed=${n.signed}, paid=${n.paid}, pdf_blob_key=${n.pdfBlobKey ?? null}, updated_at=${n.updatedAt} WHERE id=${id}`;
      return n;
    },
    ...
    async listEvents(leadId) {
      const sql = getSql();
      const r = await sql`SELECT * FROM processed_events WHERE lead_id = ${leadId} ORDER BY received_at`;
    ...
    async countClaims() {
      const sql = getSql();
      const r = await sql`SELECT COUNT(*)::int AS n FROM claims`;
      return r[0].n;
    },

    async resetSession(sessionId) {
      const sql = getSql();
      await sql`DELETE FROM claims WHERE lead_id IN (SELECT id FROM leads WHERE session_id = ${sessionId})`;
      await sql`DELETE FROM processed_events WHERE lead_id IN (SELECT id FROM leads WHERE session_id = ${sessionId})`;
